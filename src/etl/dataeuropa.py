...
print("=== DEMARRAGE DE L'ETL ===")

for ds_id in liste_datasets:
    print(f"\nTraitement de la source : {ds_id}...")
    df_temp = etl_process_europa_portal(ds_id)
    
    if df_temp is not None:
        resultats_datasets[ds_id] = df_temp
        print(f"-> Source {ds_id} enregistrée.")
    else:
        print(f"-> Source {ds_id} ignorée suite à une erreur.")
